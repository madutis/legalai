...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/lib/utils/messageConverter.ts"
      provides: "Functions to convert between ChatMessage and ConsultationMessage"
      exports: ["toChatMessage", "toConsultationMessage", "toChatMessages", "toConsultationMessages"]
  ...
  key_links:
    ...
    - from: "src/components/chat/ChatInterface.tsx"
      to: "src/lib/utils/messageConverter.ts"
      via: "converts messages on load"
      pattern: "toChatMessages"
---
...
<task type="auto">
  <name>Task 2: Create message format conversion utility</name>
  <files>src/lib/utils/messageConverter.ts</files>
  <action>
Create a dedicated utility file for converting between ChatMessage (useChat format) and ConsultationMessage (Firestore format).
...
This provides a single source of truth for message format conversion, avoiding inline conversions scattered across components.
  </action>
  <verify>`npm run build` passes, exports are correct</verify>
  <done>Message converter utility created with all 4 functions</done>
</task>

<task type="auto">
  <name>Task 3: Trigger title generation after first exchange</name>
...
<task type="auto">
  <name>Task 4: Enable loading and continuing saved consultations</name>
...
useEffect(() => {
  if (consultation?.messages && consultation.messages.length > 0) {
    // Convert ConsultationMessage[] to ChatMessage[] using utility
    const chatMessages = toChatMessages(consultation.messages);
...
- Message converter: converts correctly in both directions
...
- messageConverter utility provides clean format conversion
