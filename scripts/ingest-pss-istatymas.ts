/**
 * Ingest PSS Įstatymas (Priešgaisrinės saugos įstatymas - Fire Safety Law)
 *
 * Fetches latest consolidated version from e-TAR and ingests articles to Pinecone.
 *
 * Usage:
 *   npx tsx scripts/ingest-pss-istatymas.ts --dry-run   # Parse and show articles without ingesting
 *   npx tsx scripts/ingest-pss-istatymas.ts             # Full ingestion to Pinecone
 */
...
// e-TAR document identifiers for PSS Įstatymas
const ETAR_DOC_ID = '9CBB77180BFE';
...
async function parseDocx(buffer: Buffer): Promise<Article[]> {
  ...
  while ((match = articlePattern.exec(cleanedText)) !== null) {
    ...
    // PSS Įstatymas has about 22 articles
    if (num >= 1 && num <= 30 && title.length > 3 && !/^[)\].,;:]/.test(title)) {
...
async function ingestToPinecone(articles: Article[]): Promise<number> {
  ...
  for (let i = 0; i < articles.length; i++) {
    ...
    const textForEmbed = `PSS įstatymas ${art.num} straipsnis: ${art.title}\n\n${art.text}`;

    try {
      const embedding = await generateEmbedding(textForEmbed);
      vectors.push({
        id: `pss-istatymas-str-${art.num}`,
        values: embedding,
        metadata: {
          docId: `pss-str-${art.num}`,
          docType: 'legislation',
          lawCode: 'PSS',
          articleNumber: art.num,
          articleTitle: art.title,
          text: art.text.slice(0, 8000),
          sourceUrl: 'https://www.e-tar.lt/portal/lt/legalAct/TAR.9CBB77180BFE/asr',
          chunkIndex: 0,
          totalChunks: 1,
        },
      });
...
async function main() {
  console.log('=== PSS Įstatymas Ingestion Script ===\n');
  ...
  if (dryRun) {
    console.log('\n=== DRY RUN COMPLETE ===');
    console.log(`Would ingest ${articles.length} articles with lawCode=PSS`);
    console.log('\nArticle metadata sample:');
    if (articles.length > 0) {
      const sample = articles[0];
      console.log(`  id: pss-istatymas-str-${sample.num}`);
      console.log(`  docId: pss-str-${sample.num}`);
      console.log(`  docType: legislation`);
      console.log(`  lawCode: PSS`);
  ...
  console.log(`\n=== Done! Total PSS articles added: ${count} ===`);
}
