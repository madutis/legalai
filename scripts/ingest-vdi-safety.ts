...
async function processDocument(item: ManifestItem): Promise<DocumentChunk[]> {
  ...
  console.log(`  ${item.title.slice(0, 50)}: ${content.length} chars -> ${textChunks.length} chunks`);
...
async function ingestDocuments(
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    console.log(`Processing ${i + 1}/${chunks.length}: ${chunk.title.slice(0, 40)} (${chunk.chunkIndex + 1}/${chunk.totalChunks})`);
    ...
    if (!dryRun) {
      try {
        ...
        vectors.push({
          id: `vdi-safety-${sanitizedDocId}-chunk-${chunk.chunkIndex}`,
          values: embedding,
          metadata: {
...
async function main() {
  console.log('=== VDI Safety Document Ingestion Script ===\n');
  ...
  // Read manifest
  const manifestPath = path.join(process.cwd(), 'data', 'vdi-safety-manifest.json');
  if (!fs.existsSync(manifestPath)) {
    console.error(`Manifest not found at ${manifestPath}`);
    console.error('Run discover-vdi-safety.ts first to generate the manifest.');
    process.exit(1);
  }
  ...
  for (const item of eligibleItems) {
    console.log(`\nProcessing: ${item.title.slice(0, 60)} (Tier ${item.tier})`);
